...
const blankProfile = (cookieID: string): AIProfileInput => ({
  ...
  custom_prompts: '', thinking_mode: 'disabled', max_discount_percent: 10, max_discount_amount: 100, max_bargain_rounds: 3, item_ids: [],
});
...
const AIAssistants: React.FC<{ isAdmin: boolean }> = ({ isAdmin }) => {
  ...
  const openEdit = (profile: AIProfile) => {
    setEditing({ cookie_id: profile.cookie_id, name: profile.name, enabled: profile.enabled, use_system_api: profile.use_system_api, base_url: profile.base_url, model_name: profile.model_name, custom_prompts: profile.custom_prompts, thinking_mode: profile.thinking_mode || 'disabled', max_discount_percent: profile.max_discount_percent, max_discount_amount: profile.max_discount_amount, max_bargain_rounds: profile.max_bargain_rounds, item_ids: [...profile.item_ids] });
