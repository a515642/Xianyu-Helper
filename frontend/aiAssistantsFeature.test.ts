...
describe('product scoped AI assistants', () => {
  ...
  test('supports account isolation, item multi-select, custom API and global forbidden words', () => {
    ...
    expect(page).toContain('{!editing.use_system_api && <>');
    expect(page).toContain('<label className="flex items-center gap-3 font-bold"><input type="checkbox" checked={editing.thinking_mode === \'enabled\'}');
