...
func TestAdjustOrderPrice(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("api") != AdjustPriceAPI {
			t.Fatalf("api=%s", r.URL.Query().Get("api"))
		}
		body, _ := io.ReadAll(r.Body)
		if !strings.Contains(string(body), "orderId") || !strings.Contains(string(body), "1234") || !strings.Contains(string(body), "targetPrice") || !strings.Contains(string(body), "9900") {
			t.Fatalf("body=%s", body)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"ret":["SUCCESS::调用成功"],"data":{"ok":true}}`))
	}))
	defer server.Close()
	client := NewClient()
